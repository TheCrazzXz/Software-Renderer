...
Uint32 interpolate2(int x, int y, struct DrawVertexData* vertex1, struct DrawVertexData* vertex2)
{
	Vector2 point = Vec2_create(x, y);

	float verticesDist = Vectors2_distance(&vertex1->position, &vertex2->position);

	float Vert0Interpolant = Vectors2_distance(&point, &vertex1->position) / verticesDist;
	float Vert1Interpolant = Vectors2_distance(&point, &vertex2->position) / verticesDist;

	int color_x = (int)(COLOR_RED(vertex1->color) * Vert0Interpolant + COLOR_RED(vertex2->color) * Vert1Interpolant);
	int color_y = (int)(COLOR_GREEN(vertex1->color) * Vert0Interpolant + COLOR_GREEN(vertex2->color) * Vert1Interpolant);
	int color_z = (int)(COLOR_BLUE(vertex1->color) * Vert0Interpolant + COLOR_BLUE(vertex2->color) * Vert1Interpolant);

	color_x = clampInt(color_x, 0, 255);
	color_y = clampInt(color_y, 0, 255);
	color_z = clampInt(color_z, 0, 255);

	return pixelRGB(color_x, color_y, color_z);
}
...
bool renderInterpolateAtPosition(int x, int y, PixelBuffer* pixelBuffer)
{
	...
	return pixelAtPosition((int)x, (int)y, interpolate2(x, y, &contextVertex0, &contextVertex1), pixelBuffer);
}
