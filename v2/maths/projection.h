...
void createProjectionMatrix(float fov, float aspectRatio, float NEAR_Z, float FAR_Z) {
  ...
  Matrix4_clear(&projectionMatrix);
  tan_half_angle = tan(fov / 2);

  MATRIX_ELEMENT_VALUE(projectionMatrix, 0, 0) = 1.0f / (aspectRatio * tan_half_angle);

  MATRIX_ELEMENT_VALUE(projectionMatrix, 1, 1) = 1.0f / (tan_half_angle);
  MATRIX_ELEMENT_VALUE(projectionMatrix, 2, 2) = -(FAR_Z + NEAR_Z) / (FAR_Z - NEAR_Z);
  MATRIX_ELEMENT_VALUE(projectionMatrix, 3, 2) = -1.0f;
  MATRIX_ELEMENT_VALUE(projectionMatrix, 2, 3) = -(2.0f * FAR_Z * NEAR_Z) / (FAR_Z - NEAR_Z);
